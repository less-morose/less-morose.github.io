...
- The Windows 10 Wazuh Agent is used to aggregate logs collected with Sysmon to the Wazuh Manager. It's a VirtualBox VM hosted on my main machine where I also access the cloud consoles from.  
- Wazuh is then configured to connect to the SOAR software, Shuffle, to automatically calculate the SHA256 hash from the mimikatz.exe alert to send to VirusTotal, generate an alert for TheHive, and send a separate email to directly alert relevant emails. 

### Installation/Configuration

#### Firewall
![Desktop View](/assets/posts/SOARSIEM/digitaloceanFirewall.png){: w="200" .right }
Before initializing both the Wazuh and TheHive managers, by default port 22 (SSH) is open to allow remote connections by anyone. Obviously, for security reasons I need to either restrict access to SSH via key pair login only, or simply just close off the port to avoid scanners probing the droplets. I opted for the latter and instead use the web console provided by DigitalOcean. I also enabled all TCP/UDP connections from my public IP address for the Wazuh Agent to be able to communicate to/from the Wazuh Manager.

#### Wazuh Agent

The Wazuh Manager has a very convenient curl command to establish the Wazuh Agent. By selecting the OS version and the IP address of the manager, it automatically fills the fields and provides additional commands to start the service on the agent.  
```bash
...
Installing sysmon is even easier. By installing the the Sysmon service through the [Microsoft Documentation](https://learn.microsoft.com/en-us/sysinternals/downloads/sysmon){: target="_blank" } , and a systemconfig.xml file offered through the GitHub linked in that documentation page.
```bash
.\Sysmon.exe -i sysmonconfig.xml
```
On the Windows 10 Agent under the 'ossec.conf' file within C:\Program Files (x86)\ossec-agent, I edited the already existing location tag to the Sysmon file path (Microsoft-Windows-Sysmon/Operational) so that the Wazuh Agent sends the telemetry generated by Sysmon.
...
### Detecting Mimikatz 
Now that the log ingestion is configured and ready to go, it is time to create a custom rule that detects the mimikatz.exe. Because all archives are ingested, I can select some random log to look at the attributes that Sysmon offers.
...
- data.win.system.eventID
	- An eventID of 1 represents Process Creation. In properly made rules, this attributed used in tandem with other events to contribute to process lineage giving insights on how the potential malware works sequentially or vulnerabilities within the environment.
...
- __options__ -> Removed the tags and 'no_full_log' to ingest every single attribute for testing purposes.
...
On the Windows 10 Wazuh Agent, I installed the mimikatz.exe, changed the name of the executable, and ran it.
...
Now with the rule configured, I can configure the Wazuh Manager to only push alerts with the Mimikatz Rule ID (100002) to Shuffle.
...
I then copy the Webhook URI provided from the Webhook App that I added to the Shuffle workspace. In addition to filling out , and replaced the 'level' tag with the 'rule_id' tag with the Mimikatz Rule ID (100002), which is similar in formatting to the custom rule earlier.  

![Desktop View](/assets/posts/SOARSIEM/shuffle-wazuhman-url.png)  
To save the changes, I saved the ossec.conf file and restarted the service. Once that was done, I reran the mimikatz.exe on the Wazuh Agent, and tested the workflow to populate the alert attributes within the environment so I could start performing some automation.
...
However, Wazuh puts all of these hashes on the same variable. Looking through the apps, the only way to get one checksum from the line is Regex code within a Shuffle Tools app. I don't know how to code in Regex, so I asked ChatGPT to only get the SHA256 sum from the input. I then put this code within SHA256_Regex's last field, 'Regex'.
...
I set the SHA256_Regex output to the VirusTotal app, but first I had to set up the authentication process for it. I created an account to get an API key used to authenticate the app. Through this app, similar to getting the attributes from the mimikatz.exe rule trigger, I can see the amount of vendors that list it as malicious, as well as its overall reputation score.  
<br>
<br>

#### TheHive

Now to extend the workflow to create an alert on the TheHive manager. Before that, I need to create a new organization, as well as two user accounts: one service account to establish the API key to connect and authenticate to TheHive app, and a user account to receive the alerts I create and push through Shuffle. I used a pre-made profile which gives a variety of baseline permissions. Note that in a real environment, this would be configured with the idea of least privilege, which is not a concern for testing.
...
For the Analyst account, I established credentials to log into it. Once alerts are sent from Shuffle, I would be able to log into the account with the username, 'myorganization@test.com', and the account password to actually see them.  
<br>
On the left below, these are the initial settings I had.  
Testing the workflow I ran into the following issue on the right.
...
Looking for solutions, I browsed through the other settings and switched to the advanced tab, which is all the inputs but formatted in JSON. I had to trial and error and figure out through searching up the error codes from TheHive app, that they have documentation on required fields and their type of inputs [here.](https://docs.strangebee.com/thehive/api-docs/#tag/Alert/operation/Create%){: target="_blank"}  
...
_Final json_
After tons of testing, it finally works!
...
## Challenge - Configuring Active Response command on Vulnerable Agent
...
4. Configure more descriptive alerts to send to TheHive manager for the Analyst Account.
5. Create an active response command that adds an IP address to the host' IP tables to be dropped.
6. Configure emails within Shuffle to ask user if active response should run in response to source IP.
...
Because I opened all communications to genuine bot scanning traffic, I did not want to host another VirtualBox VM or anything on-prem to avoid DOS attacks on my home modem/router.  
![Desktop View](/assets/posts/SOARSIEM/challenge-other-droplet.png)  
I preemptively made a firewall that opens all connections, but only implemented it when testing and gathering final results.
![Desktop View](/assets/posts/SOARSIEM/challenge-other-droplet-firewall.png)
 
### Push different alerts  
 
Instead of pushing alerts that trigger the Mimikatz Rule ID (100002), I replaced the rule tag with level tags and set it to level 5 alerts.  
...
In order to configure third party apps to perform actions from Wazuh, I need to authenticate my connection via a Json Web Token (JWT). Following [this link](https://documentation.wazuh.com/current/user-manual/api/getting-started.html#understanding-the-wazuh-server-api-request-and-response){: target="_blank"} there is a curl command that can be used to establish the JWT authenticated session. The localhost would be replaced with the IP address of the Wazuh Manager.
```bash
TOKEN=$(curl -u <WAZUH_API_USER>:<WAZUH_API_PASSWORD> -k -X POST "https://localhost:55000/security/user/authenticate?raw=true")
```
This token is established alongside the 'payload,' that being the specific commands and alerts you would tell Wazuh to execute/look for on relevant agent IDs. There is an option to include the Wazuh Shuffle app within the workload itself, so the Http app (Get-Wazuh-API) acts as the TOKEN, and the Wazuh Shuffle app acts as the payload and is responsible for its delivery. I replaced the POST http request with GET as I will request data from the Wazuh Manager through the Wazuh App rather than sending the payload all at once.  
<br>
There is no inherent benefit within the scope of this project, but is just another way to establish connection to the Wazuh API. I also opened up port 55000 on the already existing firewall for the Wazuh Manager droplet in order to establish the JWT.  
...
For TheHive, I had to mess with the JSON and keep in mind the required object types for each key. Within TheHive alerts, cases are divided between source, sourceRef, and type. Essentially, if I used static values rather than dynamic $exec values, alerts would not be able to be made despite there being different IP addresses or rules triggered.  
<br>
Compared to the TheHive alerts for mimikatz, here are some notable things that I replaced:
...
Pretty much same as the mimikatz configuration, just with the source IP variable and no host name instead. The default wazuh rules do not include everything, unlike the ingested custom alert I made for the mimikatz workflow.
![Desktop View](/assets/posts/SOARSIEM/final-workflow-email.png)  

#### User input

Within Shuffle's User Input app, whenever it triggers for emails, it automatically configure two links to allow or deny the continuation of the workflow, so the inputs are very simple.  
...
within /var/ossec/etc/ossec.conf there are active-response tags that are commented out. I removed them and created the following lines to block the source IP of an alert provided it activates on least level 5.  
...
The source IP address that was chosen for this final workflow, was 194.0.234.19, originating from Iran.
![Desktop View](/assets/posts/SOARSIEM/final-result-1.png)  

### TheHive

![Desktop View](/assets/posts/SOARSIEM/final-result-2.png) 
![Desktop View](/assets/posts/SOARSIEM/final-result-2-1.png) 
...
The iptables of the ubuntu machine are blocking the source IPs, but have multiple entries that definitely clog up it up. After letting it run after documenting the picture, it was blocking domain names rather than just source IPs which was weird. Most likely has to do with the various default rules.
  
My assumption is because I configured all logs to be pushed to the Wazuh Manager, the archives do not get deleted/refreshed. So even after I flush the ubuntu machine's iptables, the workflow attempts every step for EVERY level 5 log that is pushed to the manager. Not only that, many of these port scanners most likely trigger more than one rule, creating many unnecessary entries. I also found out that many of the default active response command rules within the .xml files include the description, 'Host Blocked by firewall-drop Active Response' so I had to filter that out and realize that not including a description to my firewall-drop command was not the best idea. Better to be more verbose than less in these cases, I guess. There are logs that say an active-response command was logged originating from the ossec.conf file, so it does work.

In addition, I already figured that an active response command to each IP address ironically causes more alarm fatigue than without, but it is nice to somewhat simulate conditions without having the insight working in an enterprise environment would give.  

## Additional Comments

I used [MyDFIR's tutorial](https://www.youtube.com/watch?v=Lb_ukgtYK_U&list=PLEd_qaF8wpnXgdngqfsQtYYGM-IdtuxmC){: target="_blank"}  and tried as much as possible to not completely rely on the . I will say it's very hard to try to come up with ideas of this scale and relevance without the experience working in a technical environment. The most experience I had were school labs that pretty much felt like it held my hand where the most I can do to expand on the information, is just using the software but outside of the project and tinkering with it. I will say, after doing the project, looking through all the documentation, and all the trial and error, I can safely explain each step of this project and where I can improve on as well.  
